...
int World::spawnMob(SDL_Rect pos, unsigned int rmin, unsigned int rmax, unsigned int nbrSpawns, IniReader* dataSpawn)
{
	...
	for(unsigned int i = 0; dataSpawn->get("Spawn on_blockid_"+intToStr(i)); i++)
		idBlocks.push_back(dataSpawn->getInt("Spawn on_blockid_"+intToStr(i)));
...
void World::getMobSettingsFromData()
{
	...
	do 
	{
		m_mobSettings.push_back(IniReader( "."FILE_SEPARATOR"Levels"FILE_SEPARATOR"" + m_genericName + ""FILE_SEPARATOR"Monsters"FILE_SEPARATOR"" + intToStr(i) + ".ini"));
		i++;
