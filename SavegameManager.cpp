...
void SavegameManager::savePokemonTeam()
{
	...
	for(unsigned int i = 0; i < 6; i++)
		remove(("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Saves"FILE_SEPARATOR + m_pathname + FILE_SEPARATOR"Team" + FILE_SEPARATOR+ intToStr(i) +".ini").c_str());

	for(unsigned int i = 0; i < wScreen.getPokemonManager().getPokemonTeamSize(); i++)
	{
		ofstream of("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Saves"FILE_SEPARATOR + m_pathname + FILE_SEPARATOR"Team" + FILE_SEPARATOR+ intToStr(i) +".ini");
		of.close();
		reader.load("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Saves"FILE_SEPARATOR + m_pathname + FILE_SEPARATOR"Team" + FILE_SEPARATOR+ intToStr(i) +".ini");
		if(reader.isLoaded())
		{
			...
			reader.save("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Saves"FILE_SEPARATOR + m_pathname + FILE_SEPARATOR"Team" + FILE_SEPARATOR+ intToStr(i) +".ini");
		}
	}
}
...
void SavegameManager::loadPokemonTeam()
{
	...
	do 
	{
		reader.load("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Saves"FILE_SEPARATOR + m_pathname + FILE_SEPARATOR"Team" + FILE_SEPARATOR+ intToStr(index) + ".ini");
		if(reader.isLoaded())
		{
...
void SavegameManager::saveItems()
{
	...
	{
		reader.set("Items "+ intToStr(i) + "_id", wScreen.getInventory().getObjectFromIndex(i)->getID());
		reader.set("Items "+ intToStr(i) + "_amount", wScreen.getInventory().getAmountFromIndex(i));
	}
	reader.save("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Saves"FILE_SEPARATOR + m_pathname + FILE_SEPARATOR"trainer.ini");
}
...
void SavegameManager::loadTrainer()
{
	...
	for(unsigned int i = 0; reader.get("Items "+ intToStr(i) + "_id"); i++)
		loadItem(reader.getInt("Items "+ intToStr(i) + "_id"), (unsigned int) reader.getInt("Items "+ intToStr(i) + "_amount"));
}
