...
bool IniReader::load(string inifilename)
{
	m_content.clear();
...
void IniReader::save(string inifilename)
{
	...
	for(map<string, string>::iterator it = m_content.begin(); it != m_content.end(); ++it)
...
bool IniReader::get(string path)
{
	return (m_content.find(path) != m_content.end());
}

string IniReader::getString(string path)
{
...
int IniReader::getInt(string path)
{
	if(m_content.find(path) != m_content.end())
...
bool IniReader::getBoolean(string path)
{
	if(m_content.find(path) != m_content.end())
...
float IniReader::getFloat( std::string path )
{
	if(m_content.find(path) != m_content.end())
	{		
		stringstream ss;
