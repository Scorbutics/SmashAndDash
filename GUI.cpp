...
void GUI::resetInfoPokemonWindow(Character* pokemon)
{
    ...
    if(m_pnj != NULL)
    {
        ...
        m_pokeInfoWindow->addTextArea("Level : " + intToStr(pokemon->getStatistics()->getLevel()), 20, buf);
        buf.y = TAILLEBLOCFENETRE/2 + 80;
        m_pokeInfoWindow->addTextArea("HP : " + intToStr(pokemon->getHp()), 20, buf);
        buf.y = TAILLEBLOCFENETRE/2 + 100;
        m_pokeInfoWindow->addTextArea("Position : " + intToStr(posHero.x) + ":" + intToStr(posHero.y), 20, buf);
...
void GUI::resetInfoPNJWindow(Character* pnj)
{
    ...
    if(m_pnj != NULL)
    {
        ...
        buf.y = TAILLEBLOCFENETRE/2 + 20;
        m_pnjInfoWindow->addTextArea("Level : " + intToStr(pnj->getStatistics()->getLevel()), 20, buf);
        buf.y = TAILLEBLOCFENETRE/2 + 40;
        m_pnjInfoWindow->addTextArea("HP : " + intToStr(pnj->getHp()) + "/" + intToStr(pnj->getStatistics()->getHpMax()), 20, buf);
        buf.y = TAILLEBLOCFENETRE/2 + 60;
        m_pnjInfoWindow->addTextArea("Position : " + intToStr(posHero.x) + ":" + intToStr(posHero.y), 20, buf);
