...
DialogMenu::DialogMenu(string texte, string messImg, string fichierMenu, SDL_Rect posFond, int taillePolice, bool scroll)
{
    ...
    m_police = TTF_OpenFont(FILE_FONT, m_taillePol);
    if(m_police == NULL)
    {
...
void DialogMenu::display()
{
    ...
    m_posTexte.y = RECT_OFFSET + m_posFond.y - m_posScrollFond.y + m_posFond.y + m_taillePol/2;
...
void DialogMenu::modifyColor(SDL_Color col)
{
    ...
    for(unsigned int i = 0; i < m_size; i++)
		m_stexte[i].loadFromText(m_police, m_texte[i], m_couleur);
...
void DialogMenu::modifyText(string texte)
{
	...
	{
		m_stexte[j].loadFromText(m_police, m_texte[j], m_couleur);
