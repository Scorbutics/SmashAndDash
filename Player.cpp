	...
	else
	{
		...
		if (ke->getScancode() == SDL_SCANCODE_RETURN && !wScreen.getFight().isFighting())
		{
			...
			if (buf.y != -ENTITEMAX)
			{
				...
				if (id >= 0)
					wScreen.getGUI().getImgDialog()->setMessImg("."FILE_SEPARATOR"Sprites"FILE_SEPARATOR"Facesets"FILE_SEPARATOR"" + intToStr(id) + ".png");
				else
					wScreen.getGUI().getImgDialog()->setMessImg("."FILE_SEPARATOR"Sprites"FILE_SEPARATOR"Facesets"FILE_SEPARATOR"pnj" + intToStr(-id) + ".png");
