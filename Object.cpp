...
{
	m_id = id;
	IniReader reader("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Items"FILE_SEPARATOR+ intToStr(m_id) + ".ini");
	...
	m_spriteName = "."FILE_SEPARATOR"Sprites"FILE_SEPARATOR"Inventory"FILE_SEPARATOR + intToStr(m_id) + ".png";
...
bool Object::use()
{
    ...
    else if(m_action == "capture")
    {
		...
		if(wScreen.getPokeball().isVisible() || wScreen.getPokemonManager().getPokemonTeamSize() >= 6)
			return false;
		...
		wScreen.getPokeball().setCapture(true);
		wScreen.getPokeball().launch(wScreen.getEntityFactory().getTrainer(), mousePos);
    }
