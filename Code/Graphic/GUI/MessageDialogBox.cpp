...
MessageDialogBox::MessageDialogBox(ska::MouseObservable& gui, ska::KeyboardObservable& keyboardObs, ska::InputContextManager& icm, const std::string& text, const std::string& imageResource, const unsigned int screenH, const unsigned int screenW, const int timeout) :
m_dialog(gui, keyboardObs, text, { 0, screenH - TAILLEBLOCFENETRE * 4, screenW / 2, TAILLEBLOCFENETRE * 4 }, timeout),
m_talker(gui, keyboardObs, "", imageResource, { TAILLEBLOCFENETRE, screenH - TAILLEBLOCFENETRE * 9, 4 * TAILLEBLOCFENETRE, 5 * TAILLEBLOCFENETRE }, timeout),
m_playerICM(icm) {
	m_instanceExists++;
}
