...
namespace ska {
	...
	public:
		...
		bool organize(T& w, std::size_t cursor) {
			if (!w->isVisible()) {
				if (cursor > m_lastVisibleHandledWidget) {
					/* If we are at the end of visible widgets, we stop propagation */
					return true;
				}
				if (m_lastVisibleHandledWidget >= 0 && m_lastVisibleHandledWidget != cursor) {
					/* Otherwise this widget just turned invisible, so we swap it with the last visible widget */
					std::swap(m_data[cursor], m_data[m_lastVisibleHandledWidget]);
					m_lastVisibleHandledWidget--;
				}
			}
			else if (cursor > m_lastVisibleHandledWidget && m_lastVisibleHandledWidget + 1 < m_data.size()) {
				std::swap(m_data[cursor], m_data[m_lastVisibleHandledWidget + 1]);
				m_lastVisibleHandledWidget++;
			}
			return false;
		}
