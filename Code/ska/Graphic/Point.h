...
namespace ska {
	...
	template<>
	bool Point<float>::operator==(const Point<float>& p) const {
        return ska::NumberUtils::isLowValue(x - p.x, 0.01) && ska::NumberUtils::isLowValue(y - p.y, 0.01);
    }

	template<>
    bool Point<double>::operator==(const Point<double>& p) const {
        return ska::NumberUtils::isLowValue(x - p.x, 0.01) && ska::NumberUtils::isLowValue(y - p.y, 0.01);
    }
