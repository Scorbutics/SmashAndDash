...
namespace ska {
	...
	{
	public:
		PhysicObject(int id, unsigned int entityNumber, float weight, float frictionCoeff, unsigned int speedLimit, ska::Rectangle r);
		PhysicObject(int id, unsigned int entityNumber, float weight, float frictionCoeff, unsigned int speedLimit, ska::Rectangle r, int offset[4]);
		PhysicObject(int id, unsigned int entityNumber, float weight, float frictionCoeff, unsigned int speedLimit, ska::Rectangle r, int offset0, int offset1, int offset2, int offset3);
		...
		void applyForce(ska::Rectangle dest, float power);    				//déplacer un objet vers une destination
		...
		bool isDodging();
		bool collision();
		bool collision(ska::Rectangle targetPos);
	...
	protected:															//tout ce qui doit être accessible dans les classes filles, sans se soucier de l'accélération ou des forces
		void move(ska::Rectangle dest, float speed); //bouge dans la direction déterminée par "dest" et à la vitesse "speed"
		void move(int direction, float speed);
		void move(ska::Rectangle dest);				//bouge dans une direction vers "dest", à la vitesse courante de l'objet
		virtual std::vector<ska::Rectangle> move();	//bouge dans la direction courante à la vitesse courante
		...
		ska::Rectangle m_rect, m_lastGroundPos, m_jumpTargetPos;
