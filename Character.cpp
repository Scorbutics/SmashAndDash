...
void Character::refresh()
{
	...
	if (m_entityFollow != NULL)
	{
 		...
 		if (DistanceSquared(&entityPos, &m_rect) > 2*2*TAILLEBLOC*TAILLEBLOC)
		{
			...
			applyForce(entityPos, getSpeedLimit()*5);
		}
...
{
	return this->getStatistics()->addExperience(exp);
}
...
{
	IniReader data("."FILE_SEPARATOR"Data"FILE_SEPARATOR"Monsters"FILE_SEPARATOR + intToStr(m_id) + ".ini");
	...
	for(int i = 0; data.get("Skills " + intToStr(i)); i++)
	{
		if(data.getInt("Skills " + intToStr(i) + "_level") <= m_stats->getLevel())
			addSkill(data.getString("Skills " + intToStr(i)));
	}
    ...
    if(((int)(m_hp - damages)) <= 0)
    {
		...
		if (src != NULL)
		{
			if (!m_alive)
			{
				...
				{
					wScreen.getFight().getDialog()->modifyText(src->getDescriptor()->getName() + " monte au niveau " + intToStr(src->getStatistics()->getLevel()) + " !");
					wScreen.getFight().showDialog(2000);
				}
			...
			else if (src->isAlive())
			{
				...
				{
					wScreen.getFight().getDialog()->modifyText(getDescriptor()->getName() + " monte au niveau " + intToStr(src->getStatistics()->getLevel()) + " !");
					wScreen.getFight().showDialog(2000);
				}
			}
